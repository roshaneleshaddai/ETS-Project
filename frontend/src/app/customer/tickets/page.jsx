                        ...
                        <div className="flex items-center gap-4">
                            ...
                            <div>
                                ...
                                <p className="text-slate-500 font-medium mt-1">
                                    You have {tickets.length} active tickets across {new Set(tickets.map(t => t.eventId?._id)).size} events
                                </p>
                            </div>
                        </div>
                    ...
                    {tickets.length === 0 ? (
                        ...
                        <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
                            {tickets.map((ticket) => (
                                <div
                                    key={ticket._id}
                                    className="h-[300px] bg-white rounded-[2.5rem] overflow-hidden flex flex-col sm:flex-row shadow-lg shadow-slate-200/50 border border-slate-100 group hover:shadow-2xl hover:border-slate-200 transition-all duration-500 relative"
                                >
                                    {/* Visual "Stub" Area */}
                                    <div className="w-full sm:w-56 h-48 sm:h-auto relative overflow-hidden bg-slate-900">
                                        ...
                                        <div className="absolute bottom-4 left-4 right-4 text-white">
                                            ...
                                            <div className="flex items-center gap-2">
                                                ...
                                                <span className="font-bold">{ticket.eventId?.type || "Event"}</span>
                                            </div>
                                        </div>
                                    </div>

                                    {/* Main Ticket Area */}
                                    <div className="flex-grow p-4 flex flex-col justify-between relative bg-white">
                                        ...
                                        <div className="flex flex-col justify-center h-full">
                                            <h3 className="text-2xl font-black text-slate-900 leading-tight mb-2 line-clamp-2">{ticket.eventId?.name || "Event Name Unavailable"}</h3>

                                            <div className="space-y-2 mb-8">
                                                <div className="flex items-center gap-4 group/item">
                                                    <div className="w-10 h-10 rounded-full bg-slate-50 flex items-center justify-center group-hover/item:bg-indigo-50 transition-colors">
                                                        <Calendar className="w-5 h-5 text-slate-400 group-hover/item:text-indigo-600 transition-colors" />
                                                    </div>
                                                    <div>
                                                ...
                                                <div className="flex items-center gap-4 group/item">
                                                    <div className="w-10 h-10 rounded-full bg-slate-50 flex items-center justify-center group-hover/item:bg-indigo-50 transition-colors">
                                                        <MapPin className="w-5 h-5 text-slate-400 group-hover/item:text-indigo-600 transition-colors" />
                                                    </div>
                                                    <div>
                                                        <p className="text-xs font-bold text-slate-400 uppercase tracking-wider">Venue</p>
                                                        <p className="font-bold text-slate-900">{ticket.eventId?.location || "Venue TBA"}</p>
                                                    </div>
                                                </div>

                                                <div className="flex items-center gap-4 group/item">
                                                    <div className="w-10 h-10 rounded-full bg-slate-50 flex items-center justify-center group-hover/item:bg-indigo-50 transition-colors">
                                                        <TicketIcon className="w-5 h-5 text-slate-400 group-hover/item:text-indigo-600 transition-colors" />
                                                    </div>
                                                    <div>
                                                        <p className="text-xs font-bold text-slate-400 uppercase tracking-wider">Seat Details</p>
                                                        <p className="font-bold text-slate-900">
                                                            {ticket.zoneId?.name || "General"} • Row {ticket.seatId?.row} • Seat {ticket.seatId?.seatNumber}
                                                        </p>
                                                    </div>
                                                </div>
                                            </div>
                                        ...
                                        <div className="flex items-center gap-3">
                                            <button
                                                ...
                                                className="flex-grow flex items-center justify-center px-6 py-4 bg-slate-900 text-white rounded-xl font-bold hover:bg-slate-800 transition-all shadow-lg shadow-slate-900/10 active:scale-[0.98] disabled:opacity-70 disabled:cursor-not-allowed group/btn"
                                            >
